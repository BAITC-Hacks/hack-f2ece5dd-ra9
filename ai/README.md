...
В объединённой ветке backend вызывает этот модуль через `speech/recognizer.py` на пятисекундных WAV-фрагментах. Адаптер переводит таймкоды в секунды от начала встречи и ставит `speaker-unknown`, поскольку диаризация отсутствует. Загруженная модель кэшируется в памяти процесса; файлы модели хранятся в `models/`. `setup.cmd` устанавливает зависимости в `.venv`. Интерактивный UI для этих реплик подключается отдельно.
